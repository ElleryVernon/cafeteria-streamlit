...
def app(train, test):
    ...
    st.write("""### 1. 데이터 다운하기""")
    st.write(
        """일반적으로 회사에 속하게 된 이후에 다루게 될 데이터는 관계형 데이터베이스에 들어있고 여러 테이블이나 엑셀과 같은 파일에 나누어져 있을 것입니다. 데이터에 접근하기 위해서는 접근 권한도 있어야하고 데이터에 대한 법적 제약까지도 검토해야하죠. 하지만 지금 저희가 진행하고 있는 프로젝트는 간단합니다. 데이콘에서 진행했던 실제 대회 기반의 프로젝트이기 떄문에 모든 데이터가 들어있는 **CSV(comma-seprarated-value)** 파일이 들어있는 zip 파일만 구글 드라이브에서 다운받으면 됩니다."""
    )
    st.write(
